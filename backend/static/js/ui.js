...
export function updateFavButton() {
  ...
  btn.innerHTML = isFav(cityName) ? t.saved_fav : t.save_fav;
}
...
export function showActivityPlanner(preferredAct) {
      ...
      html += `
        <div style="flex:1; min-width:92px; background:rgba(79,195,247,0.06); border:1px solid var(--border); border-radius:12px; padding:9px 10px; font-size:0.82rem;">
          <div style="display:flex; justify-content:space-between; align-items:center;">
            <span style="font-size:1.05rem;">${d.icone}</span>
  ...
  html += `<div style="margin-top:10px; font-size:0.75rem; color:var(--text-muted); font-style:italic;">💡 ${lang === 'en' ? 'Combine these slots with the clothing and safety recommendations above.' : 'Combine ces créneaux avec les recommandations de tenue et de sécurité ci-dessus.'}</div>`;
