...
from backend.config import (
    ...
    CACHE_TTL, TILE_CACHE_TTL, CONDITION_EMOJIS
)
...
def build_forecast(forecast_data, units_param):
    ...
    for d_str, data in list(forecast_by_day.items())[:5]:
        ...
        previsions.append({
            ...
            "icone":    CONDITION_EMOJIS.get(cond, "🌡️")
        })
    return previsions
...
def parse_weather(city, unit="C", lang="fr", skip_ai=False):
    ...
    meteo = {
        ...
        "lat":                lat,
        "lon":                lon,
        "icone":              CONDITION_EMOJIS.get(main_cond, "🌡️"),
        "condition":          main_cond,
        "temperature":        temp,
        ...
        "vent":               wind_speed,
        "vent_deg":           wind_deg,
        "lever_soleil":       datetime.fromtimestamp(current["sys"]["sunrise"]).strftime("%H:%M"),
        "coucher_soleil":     datetime.fromtimestamp(current["sys"]["sunset"]).strftime("%H:%M"),
    ...
    if main_cond == "Thunderstorm":
        alerts.append({"type":"danger","icon":"⛈️","title":"Alerte Orages","msg":"Des coupures électriques ou inondations locales restent possibles."})
    elif main_cond == "Snow" and (temp < 0 if unit == "C" else temp < 32):
        alerts.append({"type":"warning","icon":"❄️","title":"Vigilance Verglas","msg":"Les chaussées peuvent être glissantes."})
    if humidity > 90:
